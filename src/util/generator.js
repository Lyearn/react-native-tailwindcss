...
function generate(name, key, values, variation = [], variantSpacing = '-') {
    ...
    for (let i = 0; i < valuesLength; ++i) {
        ...
        if (guardAgainstCssNotSupportedInReactNative(key, translateValues(value))) {
            let guardedValue = guardedKeyHandler(key, value);
            if (guardedValue) styles[styleName] = guardedValue;
            continue;
        }
        ...
        for (let j = 0; j < variationLength; ++j) {
            ...
            for (let i = 0; i < valuesLength; ++i) {
                ...
                if (guardAgainstCssNotSupportedInReactNative(keys, translateValues(value))) {
                    let guardedValue = guardedKeyHandler(keys, value);
                    if (guardedValue) styles[styleName] = guardedValue;
                    continue;
                }
...
function guardAgainstCssNotSupportedInReactNative(property, value) {
    ...
    /**
     * tailwind config adds w-max, w-min, max-w-max, min-w-max
     * react-native doesnot support { width: 'max-content' } | { width: 'min-content' }
     */
